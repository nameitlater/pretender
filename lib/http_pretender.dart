...
class Request {
  ...
  Request(this.method, this.url,
      {params, requestHeaders, queryParams, requestBody})
      : this.params = params,
...
class HttpPretender {
  ...
  HttpPretender([List<RouteMapper> maps = const []])
      : _pretender = new JsObject(context['Pretender']) {
    for (var m in maps) {
      map(m);
    }
  }
  ...
  set handledRequest(RequestHandledListener handler) {
    _pretender['handledRequest'] = (verb, path, request) {
      handler(
          _jsRequestToDartRequest(request), _jsRequestToDartResponse(request));
    };
  }
  ...
  _register(
      String method, String route, RequestHandler handler, dynamic timing) {
    if (handler != null) {
      this.handlerRequestCount[handler] = 0;
      _pretender.callMethod(method, [
        route,
        (jsr) {
...
_dartResponseToJs(Response dr) {
  if (dr != null) {
    return [dr.status, _dartHeadersToJsHeaders(dr.headers), dr.responseText];
  } else {
    return [];
  }
}
...
_jsRequestToDartResponse(JsObject jsr) {
  return new Response(jsr['status'], _jsObjectToDartMap(jsr['responseHeaders']),
      jsr['responseText']);
}
